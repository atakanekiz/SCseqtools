#' plotGseaTable2
#'
#' @description This function plots a GSEA results. The function was edited from fgsea package to enable to graph naming
#'
...
plotGseaTable2 <- function (pathways,
                           stats,
                           fgseaRes,
                           gseaParam = 1,
                           colwidths = c(5,3, 0.8, 1.2, 1.2),
                           plot_title = NULL, do.plot=T)
{
    ...
    ps <- lapply(names(pathways), function(pn) {
        ...
        list(textGrob(pn, just = "right", x = unit(0.95, "npc")),
             ggplot() + geom_segment(aes(x = p, xend = p, y = 0,
                                         yend = statsAdj[p]), size = 0.2) + scale_x_continuous(limits = c(0,
